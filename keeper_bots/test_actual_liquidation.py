...
"""
实际测试清算功能
将WLTC价格调整到刚好使净值低于清算阈值，然后启动清算机器人测试
"""
...
class ActualLiquidationTester:
    ...
    def test_actual_liquidation(self):
        """测试实际清算场景"""
        logger.info("🚀 测试实际清算场景")
            ...
            try:
                ...
                for i in range(len(token_ids)):
                    if balances[i] > 0:
                        ...
                        # 将价格调整到刚好低于清算阈值
                        # 对于MODERATE杠杆(1:4)，铸币价格300 USD
                        # 触发清算的价格是132 USD（净值=0.3000）
                        # 我们设置价格到131.99 USD，使净值略低于0.3
                        liquidation_price_usd = 131.99
...
if __name__ == "__main__":
    tester = ActualLiquidationTester()
    should_liquidate = tester.test_actual_liquidation()
    
    if should_liquidate:
        logger.info("🎯 清算条件已满足！现在可以运行清算机器人来测试清算功能。")
        logger.info("运行命令: python liquidation_keeper.py")
    else:
        logger.info("❌ 未找到满足清算条件的持仓。")
