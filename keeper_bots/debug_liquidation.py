...
class LiquidationDebugger:
    ...
    def calculate_nav_offchain(self, leverage_type: int, mint_price: int, current_price: int):
        """链下计算净值（与清算机器人相同的逻辑）"""
        try:
    ...
    def check_liquidation_status(self, user_address: str, token_id: int):
        ...
        try:
            ...
            needs_reset = status[0]
            
            logger.info(f"用户 {user_address} Token {token_id} 状态:")
            logger.info(f"  - 冻结: {is_freezed}")
            logger.info(f"  - 清算中: {is_under_liquidation}")
            logger.info(f"  - 需要重置: {needs_reset}")
    ...
    def debug_liquidation_logic(self):
        """调试清算逻辑"""
        logger.info("🔧 开始调试清算逻辑...")
        ...
        for account in accounts:
            ...
            if positions:
                logger.info(f"📊 分析账户 {account} 的持仓:")
                ...
                for position in positions:
                    ...
                    # 链下计算净值
                    nav_offchain = self.calculate_nav_offchain(
                        position['leverage_type'],
                        position['mint_price'],
                        current_price
                    )
                    
                    logger.info(f"    - 链下计算净值: {nav_offchain/10**18:.4f}")
                    logger.info(f"    - 清算阈值: {liquidation_threshold/10**18:.4f}")
                    
                    # 检查是否应该清算
                    should_liquidate = nav_offchain < liquidation_threshold
                    logger.info(f"    - 应该清算: {should_liquidate}")
                    
                    # 检查合约中的清算状态
                    status = self.check_liquidation_status(account, position['token_id'])
                    ...
                    if status:
                        ...
                        if is_freezed:
                            logger.info(f"    🚨 Token已被冻结，无法清算")
                        elif is_under_liquidation:
                            logger.info(f"    🚨 Token正在清算中")
                        else:
                            logger.info(f"    ✅ Token可以清算")
                    
                    # 如果应该清算但没有被检测到，分析原因
                    if should_liquidate and not is_freezed and not is_under_liquidation:
                        logger.info(f"    ❓ 应该清算但未被检测到，可能原因:")
                        logger.info(f"      - 链下计算与链上计算有差异")
                        logger.info(f"      - 合约中的净值计算逻辑不同")
                        logger.info(f"      - 需要检查合约中的实际清算条件")

if __name__ == "__main__":
    debugger = LiquidationDebugger()
    debugger.debug_liquidation_logic()
